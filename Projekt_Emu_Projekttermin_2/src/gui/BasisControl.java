...
public class BasisControl implements Initializable {
	...
	public void starteMessdurchlauf(final int messreihenId, MESSWERT messwert, int zeitlicheAbstandinMS) throws InterruptedException, FTDIException, NumberFormatException {
    ...
    public void speichereMessreiheInDB() throws ClassNotFoundException, SQLException{
    	try {
			...
			if(messgroesse.equals("") || verbraucher.equals("") || zeitintervall < 0)
    ...
    public void starteMessreihe() {
		...
		if (messreihe != null) {
			if (messreihe.getMessungen().length == 0){
				try {
					MESSWERT m = getMesswertVonMessreihe(messreihe);
	...
	private MESSWERT getMesswertVonMessreihe(Messreihe messreihe) {	
