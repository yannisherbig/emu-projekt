	...
	private void initListener(){
		...
		btnHoleMessungVonEMU.setOnAction(aEvent -> {
			try {
				...
				zeigeFehlermeldungAn(e1.getMessage());
			} catch (InterruptedException e1) {
				zeigeFehlermeldungAn(e1.getMessage());
			}
		});
		btnStarteMessreihe.setOnAction(aEvent -> {
			try {
				basisControl.starteMessdurchlauf(txtMessreihenId.getText());
			} catch (FTDIException e1) {
				zeigeFehlermeldungAn(e1.getMessage());
			} catch (InterruptedException e1) {
				zeigeFehlermeldungAn(e1.getMessage());
		...
		btnStoppeMessreihe.setOnAction(aEvent -> {
			basisControl.stoppeMessdurchlauf();
		});
	}
